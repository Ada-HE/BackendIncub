...
//actualizar el Usuario por el ID
app.put('/api/usuarios/:_id', async (req, res) => {
    const { _id } = req.params;
    // Incluye todos los campos que se pueden actualizar.
    const { Usuario, Nombre, APaterno, AMaterno, Telefono, CorreoE, idSecreta, respuesta, IdTipo } = req.body;

    try {
        const db = await dbPromise;
        const collection = db.collection("usuarios");
        // Asegúrate de actualizar solo los campos que deseas permitir que sean modificados.
        const updateDoc = {
            $set: {
                Usuario, // Suponiendo que deseas permitir que el nombre de usuario sea actualizable.
                Nombre,
                APaterno,
                AMaterno,
                Telefono,
...
app.post('/api/usuarios/nuevoCliente', async (req, res) => {
    ...
    try {
        ...
        const newUser = {
            ...
            AMaterno,
            Telefono,
            expiracion: new Date(new Date().getTime() + (24 * 60 * 60 * 1000)),
            idSecreta,
            respuesta,
            IdTipo: 'Cliente', 
            dispositivos: [ObjectId]
        };
...
app.post('/api/incubadoras', async (req, res) => {
    const { nombre, precio, dimension, descripcion, imagenUrl, clave } = req.body;
    ...
    try {
      ...
      const claveExistente = await collection.findOne({ Clave: clave });
      if (claveExistente) {
        return res.status(400).send("La clave ya existe, por favor usa una diferente.");
      }
  
      // Inicializa los objetos de estados y sensores con valores null o iniciales
      const estados = {
        led1: null,
        led2: null,
        tanqueExterior: null, 
      ...
      // Añade los objetos estados y sensores al objeto incubadorasData
      const incubadorasData = {
        nombre,
        precio,
        dimension,
        descripcion,
        imagenUrl,
        estados, // Agrega el objeto de estados
        sensores, // Agrega el objeto de sensores
        Clave: clave // Usa la clave proporcionada
      };
    
      const result = await collection.insertOne(incubadorasData);
      if (result.acknowledged) {
        res.status(201).send({ message: "Incubadora creada exitosamente", incubadoraId: result.insertedId, Clave: clave });
      } else {
...
app.post('/api/vincularDispositivoPorNombre', async (req, res) => {
    const { nombreUsuario, claveDispositivo } = req.body;
    ...
    try {
        ...
        // Verificar si el dispositivo existe usando la clave
        const dispositivo = await collectionDispositivos.findOne({ Clave: claveDispositivo });
        if (!dispositivo) {
            return res.status(404).send({ message: "Dispositivo no encontrado con la clave proporcionada." });
        }
        ...
        const result = await collectionUsuarios.updateOne(
            ...
            { $addToSet: { dispositivosVinculados: claveDispositivo } } 
        );

        if (result.modifiedCount === 1) {
            res.status(200).send({ message: "Dispositivo vinculado exitosamente al usuario por la clave." });
        } else {
            res.status(400).send({ message: "El dispositivo ya está vinculado a este usuario o no se pudo vincular." });
        }
    } catch (error) {
        console.error("Error al vincular el dispositivo por la clave:", error);
        res.status(500).send("Error interno del servidor.");
    }
});

// Endpoint para obtener los dispositivos vinculados a un usuario por el ID del usuario
app.get('/api/misDispositivos/:usuarioId', async (req, res) => {
    ...
    try {
        ...
        if (!ObjectId.isValid(usuarioId)) {
            return res.status(400).send("Formato de ID de usuario no válido.");
        }

        // Buscar al usuario por el ID y obtener los dispositivos vinculados
        ...
        // Suponiendo que los dispositivos están almacenados en un campo llamado 'dispositivosVinculados'
        if (usuario.dispositivosVinculados && usuario.dispositivosVinculados.length > 0) {
            // Buscar los dispositivos vinculados
            const collectionDispositivos = db.collection("incubadoras");
            const dispositivos = await collectionDispositivos.find({ 
                Clave: { $in: usuario.dispositivosVinculados }
            }).toArray();

            res.json(dispositivos);
        } else {
            res.json([]); // No hay dispositivos vinculados o el campo no existe
        }
    } catch (error) {
        console.error("Error al obtener los dispositivos vinculados:", error);
        res.status(500).send("Error interno del servidor.");
    }
});

// Endpoint para desvincular un dispositivo de un usuario
app.patch('/api/usuarios/:usuarioId/desvincularDispositivo', async (req, res) => {
    const { usuarioId } = req.params;
    const { claveDispositivo } = req.body; // La clave única del dispositivo a desvincular
  
    if (!ObjectId.isValid(usuarioId)) {
      return res.status(400).send({ message: "Formato de ID de usuario no válido." });
    }
    ...
    try {
      ...
      const result = await collectionUsuarios.updateOne(
        ...
        { $pull: { dispositivosVinculados: claveDispositivo } } // Usar $pull para eliminar un elemento del arreglo
      );
  
      if (result.modifiedCount === 1) {
        res.status(200).send({ message: "Dispositivo desvinculado exitosamente." });
      } else {
        res.status(404).send({ message: "Dispositivo o usuario no encontrado." });
      }
    } catch (error) {
...
// Endpoint para obtener una incubadora por su Clave
app.get('/api/incubadora/:claveDispositivo', async (req, res) => {
    const { claveDispositivo } = req.params;
  
    try {
        const db = await dbPromise;
        const collectionIncubadoras = db.collection("incubadoras");

        // Asegúrate de convertir la claveDispositivo a un número si es almacenado como tal en la base de datos
        const incubadora = await collectionIncubadoras.findOne({ Clave: claveDispositivo });

        if (incubadora) {
            res.json(incubadora);
        } else {
            res.status(404).json({ message: 'Incubadora no encontrada con esa clavee.' });
        }
    } catch (e) {
        console.error(e);
...
// Actualizar el estado de un LED de un dispositivo
app.patch('/api/dispositivos/comando', async (req, res) => {
    const { claveDispositivo, comando } = req.body;
    const [ledId, ledState] = comando.split(':'); // comando es un string "LED1:ON" o "LED1:OFF"

    try {
        const db = await dbPromise;
        const collection = db.collection('incubadoras');
        const update = ledState === 'ON' ? true : false;

        const result = await collection.updateOne(
            { Clave: claveDispositivo },
            { $set: { [`estados.${ledId}`]: update } } // Usa notación de corchetes para establecer el estado del LED dinámicamente
        );

        if (result.modifiedCount === 0) {
            return res.status(404).json({ mensaje: 'Dispositivo no encontrado o LED ya está en ese estado.' });
        }

        res.json({ mensaje: 'Estado del LED actualizado correctamente.' });
    } catch (error) {
        res.status(500).json({ mensaje: 'Error al actualizar el estado del LED.' });
    }
});

// Endpoint para obtener el estado de un dispositivo por su clave
app.get('/api/dispositivos/estado/:claveDispositivo', async (req, res) => {
    const { claveDispositivo } = req.params;

    try {
        const db = await dbPromise;
        const collection = db.collection("incubadoras");
        const dispositivo = await collection.findOne({ Clave: claveDispositivo });

        if (dispositivo) {
            // Asegúrate de que el objeto 'estados' exista y tenga las propiedades para cada LED
            res.json({ estados: dispositivo.estados });
        } else {
